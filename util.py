...
class AveragePrecisionMeter(object):
    ...
    def __init__(self, difficult_examples=False, workers=8):
    ...
    def value(self):
        ...
        with mp.Pool(self.workers) as pool:
            results = pool.starmap(AveragePrecisionMeter.average_precision_process, list(zip(self.scores.T.tolist(
            ), self.targets.T.tolist(), itertools.repeat(self.difficult_examples))))
        return torch.tensor(results)

    @staticmethod
    def average_precision_process(output, target, difficult_examples=True):
